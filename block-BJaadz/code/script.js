...
let imgArray = [...img];
imgArray.forEach(ele => ele.style.border = "2px solid purple")
...
let box1 = document.querySelector(".one").children;
let boxArray = [...box1];
boxArray.forEach((ele) => ele.remove());
...
let box2 = document.querySelector(".three");
let newPara2 = document.createElement('p');
newPara2.innerText("Prepend inserts as first child");
box2.prepend(newPara);
...
let allBoxesArray = [...allBoxes];
for (let i= 0; i<allBoxesArray.length; i++) {
  if(i % 2 !== 0) {
    allBoxesArray[i].style.backgroundColor = "aliceblue";
  }
} 
